...
void init_logger(bool debug_enable,  const std::string &filename, spdlog::level::level_enum level)
{
    spdlog::init_thread_pool(32768, 1);
    spdlog::flush_on(level);
    if(debug_enable) default_logger = spdlog::stdout_color_mt("default-logger");
    else default_logger = spdlog::basic_logger_mt<spdlog::async_factory>("default-logger", filename);
